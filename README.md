...
- **DocuSeal completion → `signed_pdf` artifact creation is a follow-up.** The webhook stub at `POST /api/docuseal/webhook` returns 501; the verification helper (`verify_docuseal_webhook`) and signed-PDF artifact materialization are gated on the DocuSeal version's webhook contract being finalized for v0.1.
...
- **Generated DOCX downloads through the normal contract surface.** `GET /api/contracts/{id}/download` resolves the official artifact in this order: latest `original_upload`, then latest `generated_docx`, then the legacy `Contract.s3_key` column. Generated contracts have no `original_upload`, so they serve their `generated_docx` artifact (with the artifact's filename in `Content-Disposition`). `storage_key` and `wrapped_dek` are never exposed in any response.
