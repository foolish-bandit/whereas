...
Richer policy builders, policy reconciliation/removal when requests stop matching, RBAC for policy management and overrides, deeper gate remediation links (deep-linking directly to a specific request row, workflow run, or policy detail rather than to the list page), a remediation checklist UI, request approval timeline / analytics, SLA / calendar reminders, and PowerSync sync rules are tracked as follow-ups.
...
When the gate blocks, the Contract workspace's Send-to-DocuSeal panel renders a **"How to unblock"** section that maps the gate `code` to actionable links: `active_approval_workflows` and `rejected_approval_workflows` link to the linked request's approval status (Requests page) and to the Approvals page, listing `blocking_workflow_ids` so the user can find them; `required_approval_policy_unmet` links to the Approval Policies page and shows missing policy names (falling back to ids if names are absent); `cancelled_without_completed_approval` links to the request approvals and the Approvals page so a fresh workflow can be started. Detail-level deep-linking to a specific request row, workflow, or policy is intentionally not built — the list pages are the destination for now. This is UX/explainability only; gate semantics, approval policy matching, approval state transitions, and DocuSeal send behavior are unchanged.
