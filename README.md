...
**Approval policies** (`/api/approval-policies`) are backend/API-only in this PR. Policies match requests by `request_type`, `contract_type`, `priority`, and optional linked `AgreementTemplate`; null policy fields behave as wildcards. Active matching policies can auto-attach approval workflow templates on request create/update, and policy-derived workflow runs are idempotent (tracked by source policy metadata on the run). The DocuSeal send gate now uses these backend policies to block generated/request-linked contracts when required policy approvals are unmet. If no policies match and no workflows exist, send remains allowed. Frontend policy management UI (page/form/mock mode updates) is deferred to follow-up work.
