...
- **Variable validation is small and explicit.** The backend rejects unknown keys (HTTP 400), enforces `required` variables, accepts `text` / `boolean` / `number` / `money` / `select` / `date` (`YYYY-MM-DD`) types with light coercion, and validates `select` values against `metadata_json.options` when present. The submitted values are recorded on the artifact's `metadata_json` so the audit trail answers "what did we send?".
