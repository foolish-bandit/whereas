...
from .test_findings_api import _create_workspace, _headers
...
@pytest.mark.asyncio
async def test_create_and_list_clause_template(client: httpx.AsyncClient, db_session: AsyncSession) -> None:
...
@pytest.mark.asyncio
async def test_soft_delete_excludes_by_default(client: httpx.AsyncClient, db_session: AsyncSession) -> None:
    ws = await _create_workspace(db_session)
