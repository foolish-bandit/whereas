...
# --------------------------------------------------------------------------
# Firm-authored playbook guidance fields surfaced on the API
# --------------------------------------------------------------------------


GUIDANCE_PLAYBOOK_YAML = """
name: "Firm Playbook (guidance)"
...
rules:
  - id: "governing-law-california"
    ...
    preferred_language: "This Agreement shall be governed by the laws of the State of California."
"""


class TestPlaybookGuidanceOnFindingResponse:
    async def test_response_surfaces_guidance_and_preferred_language(
        self, client: httpx.AsyncClient, db_session: AsyncSession
    ) -> None:
        ws = await _create_workspace(
            db_session, yaml_source=GUIDANCE_PLAYBOOK_YAML
        )
        response = await client.post(
            _runs_url(ws.contract.id),
            headers=_headers(ws.user),
        ...
        assert response.status_code == 201, response.text
        body = response.json()
        # Single failure: governing-law mismatch (Delaware vs California).
        assert len(body["findings"]) == 1
        finding = body["findings"][0]
        assert finding["rule_id"] == "governing-law-california"
        assert finding["guidance"] == (
            "We require California governing law for this contract type."
        )
        assert finding["preferred_language"] == (
            "This Agreement shall be governed by the laws of the "
            "State of California."
        )
        assert finding["expected_value"] == "California"
        # Per-rule projection (non-persisted) must surface the same.
        rule_result = next(
            r for r in body["results"] if r["rule_id"] == "governing-law-california"
        )
        assert rule_result["guidance"] == finding["guidance"]
        assert rule_result["preferred_language"] == finding["preferred_language"]
        assert rule_result["expected_value"] == finding["expected_value"]

    async def test_response_still_scrubs_storage_and_encryption_fields(
        self, client: httpx.AsyncClient, db_session: AsyncSession
    ) -> None:
        # Belt-and-braces: surfacing playbook guidance must not regress
        # the scrubbing guarantee on the persisted-review response.
        ws = await _create_workspace(
            db_session, yaml_source=GUIDANCE_PLAYBOOK_YAML
        )
        response = await client.post(
